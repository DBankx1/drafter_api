...
class BusinessService:
    ...
    def get_business(self, business_id: str, include_relations: bool = False) -> Optional[BusinessEntity]:
        """Get a business by ID."""
        return self.business_repo.get_by_id(business_id, include_relations)
    
    def get_business_by_subdomain(self, subdomain: str) -> Optional[BusinessEntity]:
        """Get a business by subdomain."""
        return self.business_repo.get_by_subdomain(subdomain)
    
    def create_business(self, user_id: str, business_data: BusinessCreate) -> BusinessEntity:
        """
        ...
        if self.business_repo.email_exists(business_data.email):
            raise ValueError(f"Email '{business_data.email}' is already registered")
        
        # Validate subdomain uniqueness if provided
        if business_data.subdomain and self.business_repo.subdomain_exists(business_data.subdomain):
    ...
    def update_business(self, business_id: str, business_data: BusinessUpdate) -> Optional[BusinessEntity]:
        ...
        if business_data.email and business_data.email != business.email:
            if self.business_repo.email_exists(business_data.email, exclude_id=business_id):
                raise ValueError(f"Email '{business_data.email}' is already registered")
        
        # Validate subdomain if being updated
        if business_data.subdomain and business_data.subdomain != business.subdomain:
            if self.business_repo.subdomain_exists(business_data.subdomain, exclude_id=business_id):
                raise ValueError(f"Subdomain '{business_data.subdomain}' is already taken")
        
        # Update business
        return self.business_repo.update(
            business_id,
